{-
A generator for syntactically valid javascript code.

The code is structured to be very clearly analogous to the productions in the
ECMA-262 Standard, with as few deviations as possible.  The major deviations
you will notice arise when unicode is called for.

For example, according to the standard, an Identifier can begin with a unicode
escape, e.g. \u0000.  However, no JS engines support this, so it was removed.

In the future it would be worthwhile to determine what unicode productions 
are safe in modern js engines.
